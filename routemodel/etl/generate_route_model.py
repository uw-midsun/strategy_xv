...
def generate_route_model(polyline_coordinates: list, interval_upper_bound: int) -> pd.DataFrame:
    """
    ...
    data.to_csv(os.path.normpath(os.path.join(dir_path, '../data/route_model_db.csv')), index=False)

route = []

DIR_PATH = os.path.dirname(os.path.abspath(__file__))
ROUTE_PATH = os.path.normpath(os.path.join(DIR_PATH, '../data/route.csv'))

with open(ROUTE_PATH, 'r') as csv_obj:
    csv_reader = csv.reader(csv_obj)
    list_of_rows = list(csv_reader)
    coordinates = [list(map(float, row)) for row in list_of_rows]
    route = coordinates
...
generate_route_model(polyline_coordinates=route, interval_upper_bound=25)